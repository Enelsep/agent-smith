...
def main():
    with Sandbox(timeout=2.0) as sb:
        ...
        show("11. final_answer unwinds the loop", sb.execute(
            "y = 7\nfinal_answer(f'the answer is {y}')"))
        show("12. SystemExit is forwarded, not swallowed",
             sb.execute("raise SystemExit(3)"))
        show("13. sandbox still usable afterwards",
             sb.execute("print('alive')"))
